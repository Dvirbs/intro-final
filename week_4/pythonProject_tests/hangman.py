...
def run_single_game(words_list, score):
    ...
    while check_for_another_loop(score, pattern, the_word):
        ...
        if input_type == hangman_helper.LETTER:
            if len(player_input) > 1 or not player_input.isalpha() or not player_input.islower():
...
def find_same_letters_pattern(pattern, word):
    ...
    if find_same_length(pattern, word):
        for i in range(len(pattern_note_lst)):
            if pattern_note_lst[i] == '_':
                continue
            if not pattern_note_lst[i] == word_letters_lst[i]:
                return False
        return True
    else:
        return False
...
def letter_right_place(pattern, word):
    ...
    """
    filter_word_letters = list(word)
    filter_word_letters_check = []
    for index, value in enumerate(pattern):
        if value == '_':
            if len(filter_word_letters) == len(pattern):
                filter_word_letters_check.append(filter_word_letters[index])
    if len(filter_word_letters_check) > 0:
        for val in filter_word_letters_check:
            if val in pattern:
                return False
    return True
...
def filter_words_list(words, pattern, wrong_guess_lst):
    ...
    new_words_list = []
    for word in words:
        if not find_same_length(word, pattern):
            continue
        if word_any_letters(word):
            continue
        if not wrong_letters(wrong_guess_lst, word):
            continue
        if not letter_right_place(pattern, word):
            continue
        if not find_same_letters_pattern(pattern, word):
            continue
...
def test_letter_right_place():
    if not letter_right_place('_____', 'rabit'):
        return False
    if letter_right_place('___f', 'food'):
        return False
    if letter_right_place('ll__','dell' ):
        return False
    if letter_right_place('_b_', 'bcd'):
        return False
